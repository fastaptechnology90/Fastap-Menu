...
export default function SpaPayments({ embedded = false }: { embedded?: boolean }) {
  ...
  const stats = [
    { label: "Collected (paid)", value: fmt(collectedTotal), color: "text-success", bg: "bg-success-subtle" },
    { label: "Pending amount", value: fmt(pendingAmount), color: "text-danger", bg: "bg-danger-subtle" },
    { label: "Pending bills", value: String(pending.length), color: "text-primary", bg: "bg-primary/10" },
    { label: "Total bookings", value: String(bookings.length), color: "text-info", bg: "bg-info-subtle" },
  ];
    ...
    <div className={embedded ? "space-y-5 text-foreground" : "p-4 lg:p-6 space-y-5 text-foreground"}>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search guest or service…" className="w-full bg-muted border border-border rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-primary/40 placeholder:text-muted-foreground" />
        </div>
      </div>
          ...
          {list.map(b => {
            ...
            return (
              <div key={b.id} onClick={() => tab === "history" && setDetail(b)}
                className={`bg-card border border-border rounded-lg p-4 flex items-center gap-3 ${tab === "history" ? "cursor-pointer hover:border-primary/30" : ""} transition-colors`}>
                <div className="h-10 w-10 rounded-lg bg-primary/15 flex items-center justify-center shrink-0"><Receipt className="h-5 w-5 text-primary" /></div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold truncate">{b.serviceName}</p>
                    {isPaid(b)
                      ? <span className="text-2xs font-semibold px-2 py-0.5 rounded-full bg-success-subtle text-success">PAID · {methodLabel(pay?.method)}</span>
                      : <span className="text-2xs font-semibold px-2 py-0.5 rounded-full bg-danger-subtle text-danger">PENDING</span>}
                  </div>
                ...
                <div className="text-right shrink-0">
                  <p className="text-lg font-semibold text-primary">{fmt(b.price)}</p>
                  {tab === "pending"
                    ? <button onClick={e => { e.stopPropagation(); openPay(b); }} className="mt-1 px-3 py-1.5 rounded-lg bg-success-subtle text-success text-xs font-semibold hover-elevate flex items-center gap-1"><Wallet className="h-3.5 w-3.5" /> Collect</button>
                    : <p className="text-2xs text-muted-foreground mt-0.5">by {pay?.collectedBy || "—"}</p>}
                </div>
              </div>
            );
          })}
        ...
        <div className="fixed inset-0 z-50 bg-foreground/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setPayFor(null)}>
          <div className="w-full max-w-md bg-card rounded-lg border border-border max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-border">
              <div>
                <h3 className="font-semibold flex items-center gap-2"><Wallet className="h-5 w-5 text-success" /> Collect payment</h3>
                <p className="text-xs text-muted-foreground mt-0.5">{payFor.serviceName} · {payFor.guestName}</p>
              </div>
            ...
            <div className="p-5 space-y-4">
                  ...
                  {METHODS.map(m => {
                    ...
                    return (
                      <button key={m.key} onClick={() => setForm(p => ({ ...p, method: m.key }))}
                        className={`py-2.5 rounded-lg border text-xs font-semibold flex flex-col items-center gap-1 transition-colors ${form.method === m.key ? "bg-primary/20 border-primary/40 text-primary" : "border-border bg-muted text-muted-foreground"}`}>
              ...
              <div>
                ...
                <input type="number" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground" />
              </div>
              {form.method === "upi" && (
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1">UPI ID</label>
                    <input value={form.upiId} onChange={e => setForm(p => ({ ...p, upiId: e.target.value }))} placeholder="guest@upi" className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground" />
                  </div>
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1">UTR / Ref</label>
                    <input value={form.utr} onChange={e => setForm(p => ({ ...p, utr: e.target.value }))} placeholder="UTR number" className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground" />
                  </div>
                </div>
              )}
              {form.method === "card" && (
                <div>
                  <label className="block text-xs text-muted-foreground mb-1">Card / Ref number</label>
                  <input value={form.reference} onChange={e => setForm(p => ({ ...p, reference: e.target.value }))} placeholder="Last 4 digits / approval code" className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground" />
                </div>
              )}
              <div className="flex gap-2 pt-1">
                <button onClick={() => setPayFor(null)} className="flex-1 py-3 rounded-lg border border-border hover:bg-muted text-sm font-semibold">Cancel</button>
                <button onClick={collect} disabled={saving} className="flex-1 py-3 rounded-lg bg-success hover:bg-success/90 text-background text-sm font-semibold disabled:opacity-40 flex items-center justify-center gap-2">
      ...
      {detail && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 bg-foreground/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setDetail(null)}>
            <div className="w-full max-w-sm bg-card rounded-lg border border-border max-h-[calc(100dvh-2rem)] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between p-5 border-b border-border">
                <h3 className="font-semibold flex items-center gap-2"><Receipt className="h-5 w-5 text-success" /> Payment detail</h3>
                <button onClick={() => setDetail(null)}><X className="h-5 w-5 text-muted-foreground hover:text-foreground" /></button>
              </div>
              <div className="p-5 space-y-2 text-sm">
                {[
                  ...
                  <div key={k} className="flex justify-between gap-3 border-b border-border pb-2">
                    <span className="text-muted-foreground">{k}</span><span className="font-medium text-right break-all">{v}</span>
                  </div>
                ))}
              </div>
            </div>
