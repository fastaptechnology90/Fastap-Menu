...
/**
 * There was no way to remove a customer, so a duplicate created by a typo, or a record
 * a guest asked to have deleted, could only be cleared in the database by hand.
 */
router.delete("/restaurants/:restaurantId/customers/:customerId", requireAuth, async (req, res): Promise<void> => {
  const restaurantId = parseInt(String(req.params.restaurantId), 10);
  const customerId = parseInt(String(req.params.customerId), 10);
  const [row] = await db.delete(customersTable)
